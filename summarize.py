...
def write_error(path, doc, error, num_summary):
...
if __name__ == '__main__':
    ...
    parser.add_argument('text_files', type=str, nargs="+")
    ...
    for text_file in opt.text_files:
        with open(text_file) as file:
            ...
            cnt_unk_inputs = 0
            for lst_input in inputs.tolist():
                cnt_unk_inputs += Counter(lst_input)[1]

            print("### Creating dataset pipeline ..."); sys.stdout.flush()
            inputs = tf.cast(inputs, dtype=tf.int32)
            dataset = tf.data.Dataset.from_tensor_slices((inputs)).batch(opt.batch_size)
            ...
            for idx, doc in enumerate(documents_pass):
                ...
                summary = summarize(doc,
                                    ...
                                    document_tokenizer,
                                    transformer, opt)
                summary_generated = summarize_text(
                    [summary], summary_tokenizer)[0]
                if opt.ngram_size != -1:
                ...
                write_summary(opt.path_summaries_decoded,
                                summary_generated, ind_sum + idx, 'decoder')

                #except Exception as error:
                    #write_error(opt.path_summaries_error, doc, error, ind_sum + idx)
